...
// 1. Replace renderRecruitmentForms entirely
var rrfMarker = "// ===== RENDER: 招聘类表单 =====";
var rrfStart = src.indexOf(rrfMarker);
var nextMarker = src.indexOf("// ===== RENDER: 调动类表单 =====");
if (rrfStart < 0 || nextMarker < 0) { console.log("ERROR: cant find markers"); process.exit(1); }

var newRecruitmentForms =
`// ===== RENDER: 招聘类表单（多岗位） =====
function renderRecruitmentForms() {
  ...
  formData.positions.forEach(function(pos, pi) {
    ...
    h += '<div><span style="font-size:11px;color:var(--text-secondary);">招聘计划（人）</span><div style="font-size:13px;font-weight:500;">'+(planC||'&mdash;')+'</div></div>';
    h += '<div><span style="font-size:11px;color:var(--text-secondary);">已完成招聘（人）</span><div style="font-size:13px;font-weight:500;">'+(pos.completedCount||0)+'</div></div>';
    ...
    rows.forEach(function(row, ri) {
      ...
      h += '<td><select data-pos="'+pi+'" data-idx="'+ri+'" data-field="gender"><option value="">&mdash;</option><option value="男"'+(row.gender==='男'?' selected':'')+'>男</option><option value="女"'+(row.gender==='女'?' selected':'')+'>女</option></select></td>';
  ...
  h += '<div style="text-align:center;margin-bottom:20px;"><button class="btn btn-outline" onclick="addPosition()" style="border-style:dashed;">＋ 添加拟聘岗位</button></div>';
...
function addPosition() {
  formData.positions.push({ batchId:'', batchName:'', positionIndex:-1, positionName:'', planCount:0, completedCount:0, rosterRows:[{seq:1,name:'',gender:'',birth:'',eduDegree:'',gradSchool:'',score:'',rank:'',title:'',position:'',method:'career',source:'',originalUnit:'',spouse:'',remark:''}], recordCards:[], personAttachments:{} });
  renderPage();
...
function onPositionBatchChange(pi, batchId) {
  if (!batchId) {
    formData.positions[pi].batchId = ''; formData.positions[pi].batchName = ''; formData.positions[pi].positionName = '';
    formData.positions[pi].planCount = 0; formData.positions[pi].completedCount = 0; renderPage(); return;
  }
  var batches = getCompletedRecruitBatches();
  var batch = batches.find(function(b) { return b.id === batchId; });
  formData.positions[pi].batchId = batchId; formData.positions[pi].batchName = batch ? batch.name : '';
  formData.positions[pi].positionName = ''; formData.positions[pi].planCount = 0; formData.positions[pi].completedCount = 0;
  renderPage();
  initPosNameDropdown(pi);
}
...
function initPosNameDropdown(pi) {
  ...
  try {
    ...
    var sel = document.querySelector('#posCard_' + pi + ' select[onchange*="onPositionNameChange(' + pi + ',this.value)"]');
    if (!sel) return;
    sel.innerHTML = '<option value="">— 请选择岗位 —</option>';
    var seen = {};
    batchRecs.forEach(function(rec) {
      if (!rec.formData || !rec.formData.positions) return;
      rec.formData.positions.forEach(function(p) { if (p.positionName && !seen[p.positionName]) seen[p.positionName] = p.count||0; });
    });
    Object.keys(seen).forEach(function(n) { sel.innerHTML += '<option value="' + escapeHtml(n) + '">' + escapeHtml(n) + '（计划' + (seen[n]||0) + '人）</option>'; });
  } catch(e) {}
}

function onPositionNameChange(pi, posName) {
  var pos = formData.positions[pi]; if (!pos) return;
  ...
  if (posName && pos.batchId) {
    try {
      ...
      var planC = 0;
      batchRecs.forEach(function(rec) {
        if (rec.formData && rec.formData.positions) rec.formData.positions.forEach(function(p) { if (p.positionName === posName) planC += parseInt(p.count)||0; });
      });
      pos.planCount = planC;
      try {
        var employRecs = JSON.parse(localStorage.getItem('employ_records') || '[]');
        pos.completedCount = employRecs.filter(function(r) { return r.recruitBatchId === pos.batchId && r.status === 'completed'; }).length;
      } catch(e) {}
    } catch(e) {}
  }
  renderPage();
...
function removeRosterRow(pi, ri) {
  var pos = formData.positions[pi]; if (!pos || pos.rosterRows.length <= 1) return;
  pos.rosterRows.splice(ri, 1); if (pos.recordCards) pos.recordCards.splice(ri, 1); renderPage();
}
`;

// Replace from the marker to the next marker
src = src.substring(0, rrfStart) + newRecruitmentForms + src.substring(nextMarker);

// Update function signatures and references
// openRecordCardModal(idx) -> openRecordCardModal(pi, idx)
src = src.replace(/function openRecordCardModal\(idx\)/g, "function openRecordCardModal(pi, idx)");
src = src.replace(/var row = formData\.rosterRows\[idx\];/g, "var pos = formData.positions[pi]; if (!pos) return; var row = pos.rosterRows[idx];");
src = src.replace(/var card = formData\.recordCards\[idx\] \|\| \{\};/g, "var card = (pos.recordCards||[])[idx] || {};");

// saveRecordCard
src = src.replace(/function saveRecordCard\(idx\)/g, "function saveRecordCard(pi, idx)");
src = src.replace(/formData\.recordCards\[idx\] = formData\.recordCards\[idx\] \|\| \{\};\n  Object\.keys\(card\)\.forEach/g,
...
// openPersonFileModal(idx) -> openPersonFileModal(pi, idx)
src = src.replace(/function openPersonFileModal\(idx\)/g, "function openPersonFileModal(pi, idx)");
src = src.replace(/var row = formData\.rosterRows\[idx\];/g, "var pos = formData.positions[pi]; if (!pos) return; var row = pos.rosterRows[idx];");
src = src.replace(/var files = formData\.personAttachments\[idx\] \|\| \[\];/g, "var files = (pos.personAttachments||{})[idx] || [];");

// handlePersonFileUpload
src = src.replace(/function handlePersonFileUpload\(idx\)/g, "function handlePersonFileUpload(pi, idx)");
src = src.replace(/if \(!formData\.personAttachments\[idx\]\) formData\.personAttachments\[idx\] = \[\]/g, "if (!formData.positions[pi].personAttachments) formData.positions[pi].personAttachments = {};\n    if (!formData.positions[pi].personAttachments[idx]) formData.positions[pi].personAttachments[idx] = []");

// previewPersonFile
src = src.replace(/function previewPersonFile\(rowIdx, fileIdx\)/g, "function previewPersonFile(pi, rowIdx, fileIdx)");
src = src.replace(/var files = formData\.personAttachments\[rowIdx\] \|\| \[\];/g, "var pos = formData.positions[pi]; if (!pos) return; var files = (pos.personAttachments||{})[rowIdx] || [];");

// downloadPersonFile
src = src.replace(/function downloadPersonFile\(rowIdx, fileIdx\)/g, "function downloadPersonFile(pi, rowIdx, fileIdx)");
src = src.replace(/var files = formData\.personAttachments\[rowIdx\] \|\| \[\];/g, "var pos = formData.positions[pi]; if (!pos) return; var files = (pos.personAttachments||{})[rowIdx] || [];");

// removePersonFile
src = src.replace(/function removePersonFile\(rowIdx, fileIdx\)/g, "function removePersonFile(pi, rowIdx, fileIdx)");
src = src.replace(/if \(!formData\.personAttachments\[rowIdx\]\) return;\n  formData\.personAttachments\[rowIdx\]\.splice\(fileIdx, 1\);\n  if \(formData\.personAttachments\[rowIdx\]\.length === 0\) delete formData\.personAttachments\[rowIdx\];/g,
...
// Update bindRosterTableEvents
src = src.replace(/function bindRosterTableEvents\(\) \{[\s\S]*?\n\}/,
  "function bindRosterTableEvents() {\n  document.querySelectorAll('#mainContent input[data-pos], #mainContent select[data-pos]').forEach(function(el) {\n    if (el._bound) return; el._bound = true;\n    el.onchange = function() {\n      var pi = parseInt(this.dataset.pos);\n      var idx = parseInt(this.dataset.idx);\n      var field = this.dataset.field;\n      if (isNaN(pi) || isNaN(idx) || !field || !formData.positions[pi]) return;\n      if (!formData.positions[pi].rosterRows[idx]) return;\n      formData.positions[pi].rosterRows[idx][field] = this.value;\n    };\n  });\n}");

// Remove old addRosterRow/removeRosterRow (they now take pi parameter and are defined above)
// The old versions without pi parameter need to be removed
src = src.replace(/function addRosterRow\(\)[\s\S]*?\n\}/g, "");
src = src.replace(/function removeRosterRow\(idx\)[\s\S]*?\n\}/g, "");
src = src.replace(/function updateRecordCard[\s\S]*?\n\}/g, "");
