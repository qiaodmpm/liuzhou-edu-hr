...
| v1.2 | 2026-08-07 | F07.04 本校台账概览新增待审核期间版本切换功能：校管理员在台账待审核期间可通过页内标签页切换查看「待审核版本」和「已生效版本」（上次审核通过时的快照），已生效版本为只读模式并显示黄色提示条。审核通过时系统自动保存当前台账数据为活性快照（`activeSnapshot`），供下次提交审核时校管理员对比查看。 | Claude |
