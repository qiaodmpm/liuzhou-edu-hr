...
flowchart TD
    ...
    F1 -->|启用/停用| K[点击状态切换按钮]
    K --> K1[二次确认弹框：提示停用后业务模块不可见]
    K1 -->|确认| K2[切换 status 为启用/停用]
    K2 --> F
    K1 -->|取消| F
```
...
> **功能点总览：** 本模块共 **21** 个功能点，覆盖账号管理（3）、角色权限（3）、菜单管理（1）、操作日志（1）、公告管理（5）、机构管理（4）、字典管理（4）七大功能域。
...
| 18 | 字典类型管理 | F06.18 | P1 | 字典管理 | 系统管理员 | `dict-management.html` |
| 19 | 字典项列表查看 | F06.19 | P1 | 字典管理 | 系统管理员 | `dict-management.html` |
| 20 | 新增/编辑/删除字典项 | F06.20 | P1 | 字典管理 | 系统管理员 | `dict-management.html` |
| 21 | 字典项排序与状态切换 | F06.21 | P1 | 字典管理 | 系统管理员 | `dict-management.html` |
...
- **业务规则**：
  ...
  2. 每个字典类型包含：编码（唯一标识）、名称、分组、选择模式（单选/多选）
...
- **交互规则**：
  ...
  7. 面板底部显示"新增类型"按钮
  8. 左侧菜单栏底部提供「切换演示角色」选择器，用于 Demo 阶段快速切换不同角色验证权限控制效果
...
- **功能描述**：对字典项进行增删改操作
- **业务规则**：
  1. 新增字典项：填写名称、编码，默认启用，追加到列表末尾
  2. 编辑字典项：仅可修改名称，编码创建后不可修改
...
- **业务规则**：
  1. 上移：将当前项与上一项交换排序号（sort）
  2. 下移：将当前项与下一项交换排序号（sort）
  3. 第一项不可上移，最后一项不可下移
...
|----------|----------|----------|------|------|
| `code` | 类型编码 | 文本 | ✅ | 唯一标识，如 `orgLevel`、`retireType` |
| `name` | 类型名称 | 文本 | ✅ | 如"单位级别"、"退休类型" |
| `group` | 所属分组 | 下拉选择 | ✅ | 7 个分组：机构与编制、入编与出编、岗位设置、岗位晋升、退休呈报、师德师风、系统配置 |
| `selectMode` | 选择模式 | 下拉选择 | ✅ | 单选 / 多选 |
...
| 字段标识 | 字段名称 | 字段类型 | 必填 | 说明 |
|----------|----------|----------|------|------|
| `code` | 项编码 | 文本 | ✅ | 同一类型内唯一，创建后不可修改 |
| `name` | 项名称 | 文本 | ✅ | 如"小学"、"在编" |
| `sort` | 排序号 | 数字 | ✅ | 数字越小越靠前，新增项追加到列表末尾 |
| `status` | 状态 | 文本 | ✅ | `enabled`（启用）/ `disabled`（停用），停用后业务模块不展示 |
| `updatedAt` | 更新时间 | 日期时间 | — | 格式 `YYYY-MM-DD HH:mm:ss`，精确到秒 |
...
```json
{
  "orgLevel": {
    ...
    "selectMode": "single",
    "items": [
      { "code": "futingji", "name": "副厅级", "sort": 1, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" },
      { "code": "zhengchuji", "name": "正处级", "sort": 2, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" },
      { "code": "fuchuji", "name": "副处级", "sort": 3, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" }
    ]
  },
  "retireType": {
    "name": "退休类型",
    ...
    "selectMode": "single",
    "items": [
      { "code": "normal", "name": "正常", "sort": 1, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" },
      { "code": "illness", "name": "因病", "sort": 2, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" },
      { "code": "early", "name": "提前", "sort": 3, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" },
      { "code": "extended_end", "name": "延长期满", "sort": 4, "status": "enabled", "updatedAt": "2026-08-12 10:30:00" }
    ]
  }
}
```

> 说明：`sys_dict_data` 外层对象的 key 即字典类型编码（如 `orgLevel`），类型对象内部仅存储 `name`、`group`、`selectMode`、`items` 四个字段；字典项的 `selectMode` 继承自所属类型，不在单项中冗余存储。
...
- **字典数据为空**：首次加载时自动恢复系统预置的默认字典数据（39 个类型），确保字典数据始终可用
...
| 24 | 系统管理员编辑字典项 | 修改名称，保存后表格即时更新（编码置灰不可修改） |
...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|----------|--------|
| v1.2 | 2026-08-12 | 新增字典管理（F06.18-F06.21）4 个功能点：字典类型管理、字典项列表查看、新增/编辑/删除字典项、字典项排序与状态切换。功能点总数 17→21。新增字典数据结构（sys_dict_data）、页面清单 +1（dict-management.html）、权限矩阵 +2 行、异常处理 +6 条、操作日志埋点 +4 条、验收标准 +4 条、测试场景 +8 条。更新模块范围：侧边栏「系统设置」6→7 个菜单项。新增 2.6 字典管理流程 Mermaid 图。 | 产品 |
