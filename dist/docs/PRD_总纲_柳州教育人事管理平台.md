...
| 📝 招聘岗位 | 三类招聘（自主招聘/统一公招/中高级招聘），自主招聘三级审批，其余二级审批，与编制池岗位池联动；完成后由学校录入《招聘情况统计表》、可下载存档 |
| 📎 聘用手续办理 | 学校先发填报链接，教师本人填写个人信息后，学校下拉选人申报并自动关联招聘统计表；招聘/调动两种来源，3/2套表单，二级审批+结果录入，附件4核减/暂缓 |
| 📊 编制岗位台账 | 校管理员录入→主管单位审核→通过后供其他模块使用；34字段模型，跨模块数据聚合 |
| 🔧 系统管理 | 账号管理、机构管理、菜单管理、角色权限配置、操作日志、公告管理 |
...
| 招聘岗位 | 按类型区分 | **自主招聘**：校 → 区（初审）→ 市（终审）。**统一公招/中高级招聘**：校 → 区/市（终审）。完成后 **校录入《招聘情况统计表》**（可下载存档） | **自主招聘**：校 → 市（终审）。**统一公招/中高级招聘**：校 → 市（终审）。完成后 **校录入《招聘情况统计表》**（可下载存档） |
| 聘用手续 | 二级 + 结果录入 | 校发填报链接（教师填写）→ 校选人申报 → 区初审 → **校录入结果** → 区终审确认 | 校发填报链接（教师填写）→ 校选人申报 → 市初审 → **校录入结果** → 市终审确认 |
| 退休呈报（规划中） | 待定 | 待定 | 待定 |
...
6. 聘用手续：校管理员为招聘岗位生成填报链接发给拟聘教师（教师本人填写个人信息）→ 校下拉选人办理聘用手续（自动关联招聘统计表）→ 审批通过
...
- **不支持草稿**：本项目不支持"保存草稿"功能。表单提交后直接进入待审核状态
...
| 招聘岗位 | 6 | 三类招聘（自主/公招/中高级），自主三级其余二级，与编制池岗位池联动；完成后由学校录入《招聘情况统计表》、可下载存档 | 已完成 |
| 聘用手续 | 7 | 教师信息采集（发链接教师填报）→学校选人申报、招聘/调动双来源、3/2套表单、二级审批+结果录入、附件4核减/暂缓、自动关联招聘统计表 | 已完成 |
| 编制岗位台账 | 3 | 34字段数据模型、状态(待生效/生效中)、跨模块数据聚合、编辑权限矩阵 | 已完成 |
| 系统管理 | 7 | 账号管理、机构管理、菜单管理、角色权限配置、操作日志查看、公告管理 | 已完成 |
...
| 7 | PRD_招聘岗位模块.md | 三类招聘（自主/公招/中高级）、自主三级其余二级、编制池岗位池联动、30天撤销、修改重提、《招聘情况统计表》录入与下载 | 📝 待写 |
| 8 | PRD_聘用手续模块.md | 双来源表单、附件4核减暂缓、审批流、结果录入 | ✅ 已有 |
| 9 | PRD_编制岗位台账模块.md | 34字段模型、状态管理、跨模块聚合、权限矩阵 | ✅ 已有 |
| 10 | PRD_系统管理模块.md | 账号管理、机构管理、菜单管理、角色权限、操作日志、公告管理 | ✅ 已有 |
| 11 | PRD_编外教师入职管理模块.md | 合同制教师入职列表和详情 | 📝 待写 |
| 12 | PRD_编外教师离职管理模块.md | 合同制教师离职列表和详情 | 📝 待写 |
| 13 | PRD_数据概览模块.md | 全模块数据统计仪表盘，按角色展示编制/岗位/人员/审批汇总 | 📝 待写 |
| 14 | PRD_退休呈报模块.md | 退休申请、年龄自动计算、审批 | 📝 待写 |
| 15 | PRD_师德师风举报模块.md | 举报/受理/调查/处理公示 | 📝 待写 |
