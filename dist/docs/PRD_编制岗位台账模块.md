...
- **待审核期间版本切换**：若台账存在已生效快照（即本次提交前有审核通过的历史版本），提示条下方显示版本切换标签页——「待审核版本」（当前提交的数据）和「已生效版本」（上次审核通过时的数据）。点击「已生效版本」标签时页面切换为只读模式，展示上次审核通过时的台账数据，并显示黄色快照提示条"📋 当前查看的是审核通过前已生效的版本（只读）"。点击「待审核版本」切回当前提交数据。此功能仅供校管理员在待审核期间对比查看使用，不对市/区管理员开放
- 已驳回时显示退回原因（红色背景卡片）
...
2. 补录弹框包含以下字段：
   ...
   - ④ **使用类型明细表**（多行输入，6 种使用类型，每行含 7 列）：
     - 使用类型（固定枚举）：① 统一公开招聘 / ② 自主实施招聘 / ③ 中高级人才招聘 / ④ 引进高层次人才 / ⑤ 调动 / ⑥ 其他
     - 学历、专业（大类）、从业资格等（多行文本输入，选填，≤200 字，如"本科及以上，数学类，高中教师资格证"）
     - 相关条件（多行文本输入，选填，≤200 字，如"35周岁以下，2年教学经验"）
     - 批复（数字输入，默认 0，编办批准数）
...
4. 表格底部合计行自动汇总各列数值（批复/已占用/已入编/未使用），实时更新
...
| 版本 | 日期 | 变更内容 | 变更人 |
|------|------|----------|--------|
| v1.1 | 2026-07-16 | F07.08 编制计划补录弹框优化：使用类型明细表新增「学历、专业（大类）、从业资格等」和「相关条件」两个多行文本字段（≤200字）；移除底部公式行，改为合计行汇总；弹框加宽至960px、表格采用 fixed 布局消除横向滚动条 | Claude |
| v1.2 | 2026-08-07 | ① F07.04 本校台账概览新增待审核期间版本切换功能：校管理员在台账待审核期间可通过页内标签页切换查看「待审核版本」和「已生效版本」（上次审核通过时的快照），已生效版本为只读模式并显示黄色提示条。审核通过时系统自动保存当前台账数据为活性快照（`activeSnapshot`），供下次提交审核时校管理员对比查看。② F07.05 提交变更明细修复：`computeChanges` 补录计划显示名回退逻辑优化（`offlineDocNo` → `sourceBatchId` → `计划 #id`），避免无文号的计划显示内部数字 ID；新增补录计划编辑检测（`planChanges.edited`），同 ID 计划 `approvedCount` 变更时展示为"编外补录变更：XX → YY 人"。 | Claude |
...
> 柳州教育人事管理平台 · 编制岗位台账模块 PRD · Version 1.2 · 2026-08-07
>
